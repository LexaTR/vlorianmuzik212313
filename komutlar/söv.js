exports.run = async (client, msg, args) => {
 ...
 description: (':no_entry_sign: Ya geçerli birini etiketle ya da sana sövmemi istiyosan kendini etiketle.')
  ...
  if(member.id === "265150978603352074")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (':no_entry_sign: Hoop! Orda durucan Yapımcım @Deuxcharn#0001 e sövemezsin amını sikerim.')
}})
  if(member.id === "383700145822367745")return msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`:no_entry_sign: Hoop! Birşeyler ters gitti @❅ Lexa#8413  etiketlemişsin yarram!`)
}})
